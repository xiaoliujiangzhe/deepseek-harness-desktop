...
- 通用对话历史行增加原生风格操作菜单，支持通过 Harness 官方 RPC 重命名、分叉和归档会话。
- 新增“已归档对话”中心，可跨工作区查看归档会话并选择恢复或删除；删除前二次确认，操作前自动生成维护备份。

### 修复

- 普通分叉会话不再被误判成子 Agent 而从通用历史中消失；真正的 `origin: subagent` 会话仍保持隐藏。
- 通用历史现在读取并排除 Harness 全局归档集合；归档当前会话后自动打开其他可用会话，没有剩余会话时创建空白新对话。
- 会话标题可从 `session.list` 的持久化 projection 中正确恢复。
- 启动前保守迁移旧桌面版写入的凭据 `version: "1"` / `version: '1'`，备份后改为当前 Harness 要求的数字格式，避免服务在启动页提前退出。

### 文档

- 新增内置 IDE 工作台分阶段方案，记录 Monaco 路线、成熟项目参考、安全边界与验收范围。
- 新增桌面功能插件化方案，明确纯 DSH 插件、Desktop Bridge 混合插件与 Electron 核心边界。

### 验证

- Node.js 自动化测试 65 项通过。
- 随包 Harness `0.1.1-rc.2` 校验通过。

## [0.5.4] — 2026-08-25

### 新增

- 独立“通用对话”入口增加可展开历史列表与明确的新建按钮，支持在多个通用聊天间切换。

### 修复

- 修复隐藏内部通用 Workspace 时连带隐藏全部通用 Session 的问题。
- 顶部原生“新会话”在通用聊天中提供明确反馈：当前为空白时不再重复创建，已有内容时正常新建。
- 保留沙箱 preload 约束和工具栏优先启动顺序，避免通用对话增强再次导致右侧工具栏消失。

## [0.5.3] — 2026-08-25

### 修复

- 修复 `v0.5.2` 在沙箱 preload 中加载本地 CommonJS 模块，导致桌面工具栏、外观注入和独立通用对话入口全部未初始化的问题。
- 会话恢复选择改由 Electron 主进程的受限 IPC 执行，并增加沙箱 preload 禁止本地 `require` 的防回归测试。
