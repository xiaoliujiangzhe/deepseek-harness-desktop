...
本文记录本轮已确认并落地的功能边界。当前开发版本为 `0.3.0`；安装包是否可发布仍以 `npm run dist:win` 完成并通过打包后验收为准。

## 1. DSH 插件仓库（后端已落地，正式安装包待验证）
...
- 插件中心和浏览器必须在最终安装版中测试；当前构建因 electron-builder 下载 NSIS 工具的网络权限失败，尚未完成该项。
