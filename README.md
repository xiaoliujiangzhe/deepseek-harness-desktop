...
- **检查更新（Harness 更新）**：「设置 → 通用设置」里还有一个「Harness 更新」区块，点「检查更新」比对官方仓库最新版本并提示。本项目是 fork + 补丁，**不会在应用内自动升级**；有新版本时需重打补丁后运行 `setup-harness.cmd`（详见下文「Harness 更新」）。
...
- **检查更新**：比对 vendored 源码版本与官方 `master` 分支的 `package.json` 版本；若有新版，提示「官方最新 vX（当前 vY）」。
- **升级方式**：本项目是 fork + 补丁，官方出新版时需**重打补丁**（改动的核心文件要人工合并）后运行 `setup-harness.cmd` 重建。这是维护者任务，不是用户一键操作，因为补丁可能与官方改动冲突。
