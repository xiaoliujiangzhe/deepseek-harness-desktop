...
/** User-role message: a single string of user input, or a multi-modal part array when the message carries images. */
...
/**
 * One array content part of an OpenAI-compatible multi-modal message. Absent
 * from the text-only wire path; a message that carries an image block is
 * serialized as a part array so the adapter can forward `image_url` data.
 */
