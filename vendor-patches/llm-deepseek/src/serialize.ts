...
/**
 * Serialize a content array into multi-modal parts: text blocks to `text`,
 * image blocks to `image_url`. Only called when the array carries an image,
 * so `imageData` must already hold the attachment's data URI; an image with no
 * data is a serialization error rather than silent text-flattening.
 */
function toContentParts(blocks: readonly ContentBlock[], imageData?: ReadonlyMap<string, string>): WireContentPart[] {
  const parts: WireContentPart[] = []
  for (const block of blocks) {
    if (block.type === 'text') {
      parts.push({ type: 'text', text: block.text })
    } else if (block.type === 'image') {
      const dataUri = imageData?.get(String(block.attachment.attachmentId))
      if (dataUri === undefined) {
        throw new LlmError(
          'The DeepSeek chat-completions adapter cannot serialize an image: attachment bytes are unavailable.',
          'UNSUPPORTED_CONTENT',
        )
      }
 ...
 */
export function serializeMessages(messages: Message[], imageData?: ReadonlyMap<string, string>): WireMessage[] {
  ...
  for (const message of messages) {
    ...
    if (hasImage || text.length > 0 || toolResults.length === 0) {
      wire.push({ role: 'user', content: hasImage ? toContentParts(message.content, imageData) : text })
    }
    for (const result of toolResults) {
      wire.push({
        role: 'tool',
        tool_call_id: result.toolCallId,
        // Image-bearing tool output (e.g. read_image) is forwarded as parts;
        // empty text still needs SOME content on the wire.
        content: contentHasImage(result.content)
          ? toContentParts(result.content, imageData)
