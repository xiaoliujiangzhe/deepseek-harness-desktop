...
export interface DeepSeekCatalogModel {
  ...
  maxTokens?: number
  /** Declared input modalities; omission means text-only (this route's historical default). */
  inputModalities?: ('text' | 'image')[]
}

/**
 * Structural view of the attachment reference an {@link ImageBlock} carries.
 * Deliberately un-imported: this package must not depend on `dsh-attachment`,
 * so the plugin resolves an {@link ImageBlock}'s bytes through a caller-provided
 * callback keyed by these fields.
 */
...
export interface DeepSeekAdapterOptions {
  ...
  /**
   * Load one durable image's bytes for wire serialization, or `undefined`
   * when no attachment seam is mounted. A text-only composition may omit it;
   * an image-bearing message then serializes with an `UNSUPPORTED_CONTENT` error.
   */
  resolveImage?: (ref: ImageRefLike, signal?: AbortSignal) => Promise<{ mediaType: string; data: Uint8Array }>
}
...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  /**
   * Load every image attachment referenced by the request into a data-URI
   * map keyed by attachment id. Recurses into tool-result content so a
   * `read_image` result serializes as `image_url` too. A composition with no
   * attachment seam returns an empty map, and an image-bearing request then
   * fails at serialization with `UNSUPPORTED_CONTENT`.
   */
  private async loadImageData(messages: readonly Message[], signal: AbortSignal): Promise<ReadonlyMap<string, string>> {
    const resolveImage = this.config.resolveImage
    if (resolveImage === undefined) return new Map()
    const dataUriByAttachment = new Map<string, string>()
    const collect = async (blocks: readonly ContentBlock[]): Promise<void> => {
      for (const block of blocks) {
        if (block.type === 'image') {
          const id = String(block.attachment.attachmentId)
          if (dataUriByAttachment.has(id)) continue
          const loaded = await resolveImage(block.attachment, signal)
          dataUriByAttachment.set(id, `data:${loaded.mediaType};base64,${Buffer.from(loaded.data).toString('base64')}`)
        } else if (block.type === 'tool-result') {
          await collect(block.content)
        }
      }
    }
    for (const message of messages) await collect(message.content)
    return dataUriByAttachment
  }
  ...
  private async * request(
    ...
    const imageData = await this.loadImageData(options.messages, signal)
    const body = serializeRequest(options, connection.defaults, imageData)
