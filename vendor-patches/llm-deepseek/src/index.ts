...
import type { DeepSeekCatalogModel, DeepSeekConnectionOptions, ImageRefLike } from './adapter.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  const resolveImage = attachments === undefined
    ? undefined
    : async (ref: ImageRefLike, signal?: AbortSignal): Promise<{ mediaType: string; data: Uint8Array }> => {
      // ref is structurally an ImageAttachmentRef; the cast avoids importing
      // dsh-attachment into this package (its mediaType union is narrower than
      // ImageRefLike's string).
      const stored = await attachments.readImage(ref as never, signal)
