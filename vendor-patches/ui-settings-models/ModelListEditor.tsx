...
export function ModelListEditor(props: ModelListEditorProps): ReactNode {
  ...
  /** Whether one row declares image intake. */
  const imageInputOf = (model: ModelDraft): boolean => {
    const input = model['input']
    return Array.isArray(input) && input.includes('image')
  }

  /** Set or clear image intake on one row, keeping `text` present. */
              ...
              <div className={styles['modelAdvanced']}>
                ...
                <label className={styles['modelFieldCheck']}>
                  <input
                    ...
                    disabled={disabled}
                    onChange={(event) => { toggleImageInput(index, event.target.checked) }}
                  />
